...
The data consist of 32 lines. We processed line 18 that was decided to be the line of interest. Line 21 was the secondary objective but was not processed in time.

# Data processing

We applied standard  data processing workflow in order to guarantee quality images of the subsurface. Those processing steps are generally  considered standard for any field data processing workflow. The  survey consists of 32 sail-lines available in one SEG-Y file per line. In addition, some legacy stacks and models were provided:
...
The initial model used for this final result is once again obtained from the checkshot and horizons, however modification have been made to be less reliant on the known inaccurate time migration images. 
...
The initial goal was to integrate source estimation into the SPLSRTM algorithm based on the exisiting work in [@se-splsrtm]. However, our experiemnt showed that this algorithm will require significant rework to handle data with complex source signature and strong, shallow, ocean bottom multiples. THis method currently tries to estimate the source based on the full shot record and needs to be refined to smaller subset of the data for more robust estimation.
...
All software developed as part of this project is available under the MIT license and can be accessed on the [Devito] and [JUDI] repositories. The software is well documented and provide a step-by-step guide to reproduce the results obtained in this project. Additonally, Woodside Energy has been given a one year access to [DevitoPro] and the recipes developed during this project.
...
| Data    | File name |
|:-------:|:------:|
| Line 18 CSHOT RTM | W22GAL_LINE18_FTPreSRTM_RTM_CSHOT.segy |
