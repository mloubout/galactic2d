...
The data consist of 32 lines. We processed line 18. 

# Data processing

## Source wavelet

The wavelet provided with the dataset was used for all imaging procedures. The wavelet was filtered to a [3Hz, 40Hz] frequency band using a causal Butterworth filter of order 5 and shifted by 50ms to match the data acquisition time shift.

## Deblending correction

The dataset was truncated to 9 seconds recording out of the 10  second of available data due to debelnding artifacts in the tail end of the shot record for over 50% of the shots in ine 18. 

## Filtering

The data qas filtered between 3 Hz and 40 Hz to remove strong low frequency noise. We used a causal Butterworth filter fo order 5 to match the source filtering.

## 2D correction

Amplitude correction was performed on the field data for 2D imaging. A time gain of ordre one was applied to all traces with a 50ms shift to compensate for the recording 50ms delay in the data.
...
![](./figures/final/RTM_start_novp_0524.png)
...
We now show the image obtained with the final FWI velocity model. We display drastic improvement in focusing and continuity over the section and the noise in the deeper part of the section is greatly reduced. The main even at 4km depth is now well resolved and continuous. The shallow section is also much better focused and continuous. The impedance image is now lining up with the velocity model below 2km depth. We can however, as expected, see an imprint of the multiples below 4km with ringing and repeated events. We expect to improve on these features in the future with more advanced physics and multiple elimination methods.

::: {#fig-rtmc-v-final layout-nrow=2}

![](./figures/final/RTM_xwi-54-125_novp_0524.png)
...
Model-space sparsity promoting least square migration is an iterative imaging methods that uses the Linearized Bregman algorithm with the Curvelet transform as a sparsifying transform. The algorithm is designed to use a small subset of the dataset at each iteration leading to a total imaging cost equivalent to a single RTM. We intend to show that this advanced method leads to accurate true amplitude subsurface image at a reasonable computational cost.
...
put refs to precon paper and splsrtm paper
...
https://library.seg.org/doi/epub/10.1190/1.3124753
