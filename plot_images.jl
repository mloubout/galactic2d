...
for (resname, name) in zip(rtms, names)
    ...
    # plot_velocity(log10.(Il[startx:lasti+startx-1, 1:nz])', spacing; cmap="terrain", d_scale=0,
    #               name="", cbar=true, new_fig=false)
    # savefig("$(figpath)/illum_$(name)", bbox_inches = "tight", dpi = 150)

    # figure(figsize=(30, 12));
    # plot_velocity(log10.(Ilu[startx:lasti+startx-1, 1:nz])', spacing; cmap="terrain", d_scale=0,
    #               name="", cbar=true, new_fig=false)
    # savefig("$(figpath)/illumU_$(name)", bbox_inches = "tight", dpi = 150)

    # figure(figsize=(30, 12));
    # plot_velocity(log10.(Ilv[startx:lasti+startx-1, 1:nz])', spacing; cmap="terrain", d_scale=0,
    #               name="", cbar=true, new_fig=false)
    # savefig("$(figpath)/illumV_$(name)", bbox_inches = "tight", dpi = 150)
    ...
    figure(figsize=(30, 12));
    plot_simage((rtm[startx:lasti+startx-1, 1:nz])' ./ (Il[startx:lasti+startx-1, 1:nz]' .+ 1e-6), spacing;
                cmap=cmap, d_scale=0, perc=95, new_fig=false, name="")
    plot_simage(dvp[1:lasti, 1:nz]', spacing;
                cmap="cet_CET_L1", d_scale=0, perc=perc,  new_fig=false, name="", alpha=.5)
...
if plot_sso
    ...
    res = deserialize(rtm_xwi_125)
    rtm, Ilu, Ilv, spacing = res.rtm, res.Ilu, res.Ilv, reverse(res.spacing)
    Il = Ilv
    vp = res.m.^(-.5)
    nz = size(vp, 2)

    sso = deserialize("$(data_path)sso_p64_line_18_0524.bin")
    lasti_sso = size(sso.sso, 2)
    nh = size(sso.sso, 1) ÷ 2

    figure(figsize=(30, 12));
    plot_simage(sso.sso[nh, 1:lasti_sso, :]' ./ (Il[1:lasti_sso, 1:nz] .+ 1e-6)', spacing;
                cmap=cmap, d_scale=0, perc=perc, new_fig=false)
    savefig("$(figpath)/SSO_novp", bbox_inches = "tight", dpi = 150)
    plot_velocity(vp[1:lasti_sso, 1:nz]', spacing; cmap=vcmap, perc=perc, new_fig=false, alpha=.5, vmax=vp_max)
    savefig("$(figpath)/SSO", bbox_inches = "tight", dpi = 150)
    ...
    ix = Int(div(45000, 12.5)) #div(lasti_sso, 3)
    iz = Int(div(3000, 12.5)) #2*div(nz, 3)
    h0 = sso.offsets[1]
    d = model.d

    rtm = sso.sso[nh, 1:lasti_sso, :] ./ (Il[1:lasti_sso, 1:nz] .+ 1e-6)
    n = size(rtm)

    ogz = sso.sso[:, :, iz] ./ (Il[1:lasti_sso, iz]' .+ 1e-6)
    ogx = sso.sso[:, ix, :] ./ (Il[ix, 1:nz]' .+ 1e-6)
    ...
    axs[2, 2].set_xlabel("")
    sca(axs[2, 1])
    plot_simage(rtm', d; new_fig=false, name="", cmap=cmap, d_scale=0, perc=perc)
    ...
    savefig("$(figpath)/SSO_ofs", dpi=150, bbox_inches=:tight)
